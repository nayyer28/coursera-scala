...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    ...
    } yield (block, move :: history)).to(LazyList)
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(List((startBlock, Nil)).to(LazyList), Set.empty)
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = {
    for{
      case(block, path) <- pathsFromStart
      if done(block)
    } yield (block, path)
  }
