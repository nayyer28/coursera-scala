...
object Anagrams extends AnagramsInterface:
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(l=>l).map((k,v)=> (k,v.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    val pairedOccs:Map[Char, List[(Char, Int)]] = pairs groupBy ((char, int) => char)
    val pairedOccsSorted = pairedOccs.toList.sortBy(_._1)
    val listOccs:List[List[(Char, Int)]] = pairedOccsSorted map ((char, list) => list)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val mapX = x.toMap
    val mapY = y.toMap
    mapY.foldLeft(mapX)((xx,yy) => {
      if xx(yy._1) == yy._2 then xx - yy._1
      else xx + (yy._1->(xx(yy._1)-yy._2))
    }).toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    if sentence.isEmpty then List(List())
    else
      def sentenceAnagramsHelper(occurrences: Occurrences): List[Sentence] =
        if occurrences.isEmpty then List(List())
          ...
          for{
            ...
            sentence <- sentenceAnagramsHelper(subtract(occurrences, occurrence))
          } yield word::sentence
