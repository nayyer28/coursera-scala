...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    if (!history.contains(Move.Down) && b.down.isLegal){
      (b.down, Move.Down::history) #:: neighborsWithHistory(b, Move.Down::history)
    }
    else if (!history.contains(Move.Up) && b.up.isLegal){
      (b.up, Move.Up::history) #:: neighborsWithHistory(b, Move.Up::history)
    }
    else if (!history.contains(Move.Left) && b.left.isLegal){
      (b.left, Move.Left::history) #:: neighborsWithHistory(b, Move.Left::history)
    }
    else if (!history.contains(Move.Right) && b.right.isLegal){
      (b.right, Move.Right::history) #:: neighborsWithHistory(b, Move.Right::history)
    }
    else LazyList.empty[(Block, List[Move])]
