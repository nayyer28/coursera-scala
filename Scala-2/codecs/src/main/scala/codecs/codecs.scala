...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction {
      case Json.Num(num) if num.isValidInt => num.toInt
      case Json.Str(num) if Try(num.toInt).toOption.isDefined => num.toInt
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] = Decoder.field[String]("name")
