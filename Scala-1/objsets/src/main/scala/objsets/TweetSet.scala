...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList =
    if mostRetweeted.retweets != 0
    then new Cons(mostRetweeted, remove(mostRetweeted).descendingByRetweet)
    else Nil
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    val lMostTweeted = left.mostRetweeted
    val rMostRetweeted = right.mostRetweeted
    if elem.retweets >= lMostTweeted.retweets && elem.retweets > rMostRetweeted.retweets then elem
    else if lMostTweeted.retweets >= rMostRetweeted.retweets then lMostTweeted
    else rMostRetweeted
